...
describe('GraphQLController Integration Tests', () => {
	...
	it(`Should successfully save a configuration file 
      to the session when one is posted`, async () => {
		await request(server.app)
			.post('/api/v1/graphql/proj1/configuration')
			.attach('configuration', join(graphqlpath, 'configuration.json'))
			.set('Accept', 'application/json')
			.expect(200);
	});

	it(`Should return an error when a file 
      with an invalid name is posted to /configuration`, async () => {
		await request(server.app)
			.post('/api/v1/graphql/proj1/configuration')
			.attach('resolvers', join(graphqlpath, 'configuration.json'))
			.set('Accept', 'application/json')
			.expect(res =>
				expect(res.body.errors).toBe(
					'Expected the following files: configuration, but they were not found'
				)
			)
			.expect(400);
	});
	...
	it(`Should return an error when a file
      with an invalid extension is posted to /configuration`, async () => {
		await request(server.app)
			.post('/api/v1/graphql/proj1/configuration')
			.attach('configuration', join(graphqlpath, 'resolvers.js'))
			.set('Accept', 'application/json')
			.expect(res =>
				expect(res.body.errors).toBe(
					'Invalid file extension, only files with the following extensions accepted: .json'
	...
	it(`Should return an error when a file with invalid values is posted 
      to /configuration`, async () => {
		await request(server.app)
			.post('/api/v1/graphql/proj1/configuration')
			.attach('configuration', join(graphqlpath, 'invalidConfiguration.json'))
			.set('Accept', 'application/json')
			.expect(res =>
				expect(res.body.errors).toBe(
					"Key 'host' has invalid value. Only string values are supported"
				)
			)
			.expect(400);
	});
