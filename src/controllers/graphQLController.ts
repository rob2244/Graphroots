...
class GraphQLController {
	...
	@Middleware([
		validators.fileValidator(['configuration'], ['.json']),
		validators.configurationValidation
	])
	configuration(req: Request, res: Response) {
		const { project } = req.params;
		createProjectIfNotExists(req.session, project);

		const { data, name } = req.files.configuration as UploadedFile;
		const content = data.toString('utf-8');

		const config: CodeFile = {
			filename: name,
			content,
			type: FileType.Configuration
		};

		req.session[project].configuration = config;
		res.sendStatus(OK);
	}
}
